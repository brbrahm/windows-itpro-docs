...
The ApplicationControl CSP can also be managed locally from PowerShell or via MEM Configuration Manager's (MEMCM) task sequence scripting by leveraging the [WMI Bridge Provider](https://docs.microsoft.com/windows/client-management/mdm/using-powershell-scripting-with-the-wmi-bridge-provider).
